...
description: Extract annual Dividends Per Share for one or more public company tickers (comma-separated) by browsing Yahoo Finance in Chrome. Produces two markdown tables per ticker - (1) raw annual dividends for the past 10 years and (2) YoY growth rate. Use when the user wants annual dividend data, dividend history, payout trends, or dividend growth for a stock.
...
# Annual Dividend Table Builder

Extract and display annual Dividends Per Share for **$0** covering the past 10 fiscal years by browsing Yahoo Finance in Chrome.
...
Use `evaluate_script` to extract the table headers and dividend rows in one call:
...
5. Parse each date to extract the **fiscal year-end date** and the **year**.
6. Filter to the **past 10 fiscal years** only. If the current year is incomplete or shows "--", still include it and mark as "N/A".
7. Dividend values of `"--"` should be treated as missing/N/A (the company may not have paid a dividend that year).
...
## Step 5: Build Table 1 — Annual Dividends Per Share
...
- Show "N/A" for years with no dividend or missing data.

## Step 6: Build Table 2 — YoY Dividend Growth

Calculate YoY growth as: `(Current Year DPS - Prior Year DPS) / |Prior Year DPS| * 100`

Use the **absolute value** of Prior Year DPS in the denominator to handle edge cases.

Present a markdown table:

```
| Fiscal Year End | Dividend Per Share | YoY Growth |
|-----------------|--------------------|------------|
| 12/31/2024      | $X.XX              | +X.X%      |
| 12/31/2023      | $X.XX              | -X.X%      |
| ...             | ...                | ...        |
...
- If either the current or prior year dividend is missing/zero, show "N/A" for that growth cell.
...
- Whether the company has a consistent dividend history or if there are gaps (years with no dividend).
...
- The data source: Yahoo Finance $0 Financials (Dividends tab, Dividends Per Share).
