---
name: annual-capex
description: Extract annual Capital Expenditure for one or more public company tickers (comma-separated) by browsing Yahoo Finance in Chrome. Produces two markdown tables per ticker - (1) raw annual CapEx for the past 10 years and (2) YoY growth rate. Use when the user wants annual CapEx data, capital expenditure history, or investment spending trends for a stock.
...
2. Navigate to `https://finance.yahoo.com/quote/$0/cash-flow/`
...
- `values`: Array starting with `["Capital Expenditure", "<TTM value>", "<annual value>", ...]`

## Step 4: Parse and Organize Data

1. **Skip** the first header ("Breakdown") and first value ("Capital Expenditure").
2. **Skip** the TTM column (second header / second value) if present.
3. Pair each remaining header (date string like `"12/31/2024"`) with its corresponding CapEx value.
